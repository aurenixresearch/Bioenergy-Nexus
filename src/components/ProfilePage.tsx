...
export default function ProfilePage({ user, onNavigateToView, theme, initialProfile }: ProfilePageProps) {
  const [profileData, setProfileData] = useState<UserProfile | any>(initialProfile || null);
  ...
  if (loading && !profileData) {
    return (
      <div className="w-full min-h-[70vh] py-10 px-4 sm:px-8 max-w-5xl mx-auto animate-pulse space-y-6 text-left" id="profile_loading_state">
        <div className="h-56 bg-slate-200/70 dark:bg-slate-800/60 rounded-3xl" />
        <div className="flex items-center gap-4">
          <div className="w-20 h-20 rounded-full bg-slate-200/70 dark:bg-slate-800/60 shrink-0" />
          <div className="space-y-2 flex-1">
            <div className="h-6 bg-slate-200/70 dark:bg-slate-800/60 rounded-xl w-1/3" />
            <div className="h-4 bg-slate-200/50 dark:bg-slate-800/40 rounded-lg w-1/2" />
          </div>
        </div>
      </div>
    );
