...
========================================================================`;
  ...
  return (
    <div className="space-y-6" id={`portfolio_mgmt_${currentTab}`}>
      ...
      {/* RESEARCH ABSTRACT REVIEW DRAWERS */}
      {viewingItem && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-xs">
          <motion.div
            initial={{ opacity: 0, scale: 0.95 }}
          ...
          >
            <div className="flex justify-between items-start border-b border-slate-100 dark:border-slate-800 pb-3">
              <div>
                ...
                <h3 className="text-sm font-bold text-slate-800 dark:text-slate-100 font-display mt-1">{viewingItem.title}</h3>
              </div>
              <button onClick={() => setViewingItem(null)} className="p-1 text-slate-400 hover:text-slate-600 rounded-lg cursor-pointer">
                <X className="w-5 h-5" />
              </button>
            </div>
            ...
            <div className="space-y-3">
              ...
              <div className="space-y-1.5">
                <label className="block text-[10px] font-mono text-slate-400 uppercase tracking-wider font-bold">Research Abstract</label>
                <div className="p-4 bg-slate-50 dark:bg-slate-800/40 border border-slate-100 dark:border-slate-800 text-xs text-slate-600 dark:text-slate-300 leading-relaxed rounded-2xl h-48 overflow-y-auto custom-scrollbar">
                  {viewingItem.abstract}
                </div>
              </div>

              {viewingItem.feedback && (
                <div className="p-3 bg-red-50 text-red-800 rounded-xl border border-red-100 text-xs font-medium">
              ...
              {viewingItem.status === 'Pending' && (
                <>
                  ...
                  <button
                    ...
                    className="px-4 py-2 bg-red-50 hover:bg-red-100 text-red-700 text-xs font-bold rounded-xl cursor-pointer"
                  >
                    Reject Submission
                  </button>
                </>
      ...
      {/* COMPREHENSIVE FULL RESEARCH VIEWER MODAL */}
      {viewingFullResearch && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-xs overflow-y-auto">
          <motion.div
            ...
            className="w-full max-w-2xl bg-white dark:bg-slate-900 border border-slate-100 dark:border-slate-800 rounded-3xl p-6 shadow-2xl space-y-5 my-8 relative"
          >
            {/* Top Close Indicator */}
            <button
              onClick={() => setViewingFullResearch(null)}
              className="absolute top-4 right-4 p-2 text-slate-400 hover:text-slate-600 dark:hover:text-slate-200 hover:bg-slate-50 dark:hover:bg-slate-800 rounded-xl cursor-pointer transition-colors duration-200"
            >
              <X className="w-5 h-5" />
            </button>
            ...
            <div className="border-b border-slate-100 dark:border-slate-800 pb-4 pr-8">
              ...
              <h3 className="text-base font-extrabold text-slate-900 dark:text-slate-50 mt-2 font-display leading-snug">
            ...
            <div className="grid grid-cols-3 gap-3">
              <div className="p-3 bg-slate-50 dark:bg-slate-800/40 border border-slate-100 dark:border-slate-800 rounded-2xl text-center space-y-1 hover:border-slate-200 dark:hover:border-slate-700 transition duration-150">
                <span className="block text-[10px] font-mono uppercase text-slate-400 tracking-wider">Views</span>
                <span className="block text-sm font-black font-mono text-slate-800 dark:text-slate-100">{viewingFullResearch.views}</span>
              </div>
              <div className="p-3 bg-slate-50 dark:bg-slate-800/40 border border-slate-100 dark:border-slate-800 rounded-2xl text-center space-y-1 hover:border-slate-200 dark:hover:border-slate-700 transition duration-150">
                <span className="block text-[10px] font-mono uppercase text-slate-400 tracking-wider">Downloads</span>
                <span className="block text-sm font-black font-mono text-slate-800 dark:text-slate-100">{viewingFullResearch.downloads}</span>
              </div>
              <div className="p-3 bg-slate-50 dark:bg-slate-800/40 border border-slate-100 dark:border-slate-800 rounded-2xl text-center space-y-1 hover:border-slate-200 dark:hover:border-slate-700 transition duration-150">
                <span className="block text-[10px] font-mono uppercase text-slate-400 tracking-wider">Citations</span>
                <span className="block text-sm font-black font-mono text-slate-800 dark:text-slate-100">{viewingFullResearch.citations}</span>
              </div>
            </div>
            ...
            <div className="space-y-4 min-h-48">
              {modalTab === 'overview' && (
                <div className="space-y-3">
                  <div className="space-y-1">
                    ...
                    <div className="p-4 bg-slate-50 dark:bg-slate-800/20 border border-slate-100 dark:border-slate-800 text-xs text-slate-600 dark:text-slate-300 leading-relaxed rounded-2xl h-48 overflow-y-auto custom-scrollbar">
              ...
              {modalTab === 'citations' && (
                <div className="space-y-3">
                  <p className="text-[11px] text-slate-400">
                    Use these standardized formats for academic and environmental policy bibliography references.
                  </p>
                  <div className="space-y-3">
                    {/* APA Citation */}
  ...
  pages = {112-125}
                ...
                <form onSubmit={handleSaveFullResearchEdit} className="space-y-3.5">
                  ...
                  <div className="flex justify-end gap-2 pt-2">
                    <button
                      type="button"
                      onClick={() => setModalTab('overview')}
                      className="px-4 py-2 text-xs font-bold text-slate-500 hover:text-slate-700 dark:hover:text-slate-300 rounded-xl cursor-pointer"
                    >
                      Cancel
                    </button>
                    <button
                      type="submit"
                      className="px-5 py-2 bg-emerald-600 hover:bg-emerald-700 text-white text-xs font-bold rounded-xl cursor-pointer shadow-xs transition duration-150"
                    >
            ...
            <div className="pt-4 border-t border-slate-100 dark:border-slate-800 flex flex-wrap justify-between items-center gap-3">
              ...
              <div className="flex-grow max-w-xs">
                {downloadProgress !== null ? (
                  ...
                  <button
                    ...
                    className="px-4 py-2 bg-emerald-600 hover:bg-emerald-700 text-white text-xs font-bold rounded-xl flex items-center gap-1.5 cursor-pointer transition-all duration-200 shadow-xs"
              ...
              <div className="flex items-center gap-2">
                {viewingFullResearch.status === 'Pending' && (
                  <>
                    <button
                      ...
                      className="px-4 py-2 bg-emerald-100 hover:bg-emerald-200 text-emerald-800 dark:bg-emerald-950 dark:hover:bg-emerald-900 dark:text-emerald-300 text-xs font-bold rounded-xl cursor-pointer transition-colors duration-200"
                    >
                      Approve publication
                    </button>
                    <button
                      ...
                      className="px-4 py-2 bg-red-100 hover:bg-red-200 text-red-800 dark:bg-red-950 dark:hover:bg-red-900 dark:text-red-300 text-xs font-bold rounded-xl cursor-pointer transition-colors duration-200"
                    >
                      Reject Submission
                    </button>
                  </>
                ...
                <button
                  onClick={() => setViewingFullResearch(null)}
                  className="px-4 py-2 bg-slate-100 hover:bg-slate-200 text-slate-700 dark:bg-slate-800 dark:hover:bg-slate-750 dark:text-slate-200 text-xs font-bold rounded-xl cursor-pointer transition-colors duration-200"
                >
                  Close Viewer
                </button>
              </div>
